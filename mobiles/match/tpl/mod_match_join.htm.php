...
<div class="match-join">
  ...
  <form name="frm_matchjoin" id="frm_matchjoin" action="<?php echo U('match/join')?>" method="post" onsubmit="return false">
    ...
    <div class="row">
    <select name="province" id="frm_province" class="inpsel row-ele" onchange="change_province(this)" style="width: 49%;">
      <option value="0">请选择省份▼</option>
<?php foreach ($province AS $it):?>
      <option value="<?=$it['locaid']?>"><?=$it['location']?></option>
<?php endforeach;?>
    </select>
    <select name="city" id="frm_city" class="inpsel row-ele" style="width: 49%;margin-left: 2%;">
    ...
    <p class="row"><input name="submit" type="submit" class="btn btn-green" id="frm_submit" value="提 交" /></p>
    <p class="row">&nbsp;</p>
  </form>
</div>
